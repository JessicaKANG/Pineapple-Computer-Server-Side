...
public class DBAddcomponent implements DBConfig{
	/**
     * 使用MySQL数据源获得数据库连接对象
     *
    ...
    public static boolean exists(String componentID) {
        ...
        String sql = "select id_component from tb_component where id_component = '" + componentID + "';";// 定义查询语句
    ...
    public static boolean save(Component component) {
        QueryRunner runner = new QueryRunner();// 创建QueryRunner对象
        String sql = "insert into tb_component (id_component, name_component, type_component,price_component) values (?, ?, ?, ?);";// 定义查询语句
        Connection conn = getConnection();// 获得连接
        Object[] params = { component.getId(), component.getName(), component.getType(),component.getPrice()};// 获得传递的参数
        try {
